...
import type { ArchitectQuestion, WorldSkeleton, GenerationConfig } from "../../agents/schemas";
import type { ClarificationRequest, ClarificationResponse, ClarificationOption } from "@shared/types/human-in-the-loop";
...
function buildQuestionsRequest(
  ...
  return {
    ...
    type: "clarification",
    context: {
      title: isRussian ? "Уточнение деталей мира" : "World Details Clarification",
      ...
      reason: isRussian ? "Недостаточно данных для генерации" : "Insufficient data for generation",
    },
...
function buildSkeletonApprovalRequest(
  ...
  const configOptions: ClarificationOption[] = [
    { value: "hasFactions", label: isRussian ? "Фракции и Политика" : "Factions & Politics", description: isRussian ? "Гильдии, ордена, правительства" : "Guilds, orders, governments" },
    { value: "hasLocations", label: isRussian ? "Локации и География" : "Locations & Geography", description: isRussian ? "Города, руины, ландшафт" : "Cities, ruins, terrain" },
    { value: "hasRaces", label: isRussian ? "Расы и Виды" : "Races & Species", description: isRussian ? "Эльфы, пришельцы, мутанты" : "Elves, aliens, mutants" },
    { value: "hasHistory", label: isRussian ? "История и Хронология" : "History & Timeline", description: isRussian ? "Важные события прошлого" : "Key past events" },
    { value: "hasMagic", label: isRussian ? "Магия и Технологии" : "Magic & Technology", description: isRussian ? "Магическая система или тех-уровень" : "Magic system or tech level" },
    { value: "hasCharacters", label: isRussian ? "Ключевые Персонажи" : "Key Characters", description: isRussian ? "Важные NPC" : "Important NPCs" },
  ];
  ...
  return {
    ...
    context: {
      ...
      currentNode: "architect",
      reason: isRussian ? "Требуется утверждение структуры" : "Structure approval required",
    },
    fields: [
      {
        id: "modules",
        ...
        description: isRussian ? "Выберите, какие аспекты мира нужно сгенерировать" : "Select which world aspects to generate",
        required: true,
      ...
      },
      {
        ...
        description: isRussian ? "Оставьте пустым, если всё устраивает" : "Leave empty if everything looks good",
        required: false,
      },
      {
        id: "action",
        ...
        required: true,
        options: [
          { value: "approve", label: isRussian ? "Утвердить и Генерировать" : "Approve & Generate" },
          { value: "refine", label: isRussian ? "Переделать Скелет" : "Remake Skeleton" },
        ],
...
export async function architectNode(
  ...
  while (iteration < MAX_ITERATIONS) {
    ...
    if (!response.is_ready && response.questions) {
      const request = buildQuestionsRequest(response.questions, state.outputLanguage);
      ...
      if (userResponse.skipped) {
        ...
        for (const [key, value] of Object.entries(userResponse.answers)) {
           // Find question text if possible, or just use key/value
    ...
    if (response.is_ready && response.skeleton) {
      const request = buildSkeletonApprovalRequest(response.skeleton, state.outputLanguage);
      const userResponse = interrupt(request) as ClarificationResponse;
      ...
      if (action === "approve") {
        ...
        currentInfo.push(`Approved World Skeleton:\nTitle: ${response.skeleton.title}\nSynopsis: ${response.skeleton.synopsis}\nTone: ${response.skeleton.tone}\nThemes: ${response.skeleton.key_themes.join(", ")}`);
