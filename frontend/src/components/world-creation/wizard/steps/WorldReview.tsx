...
export const WorldReview: React.FC<Props> = ({ data, sessionId, onSave, onError }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("Не удалось сохранить мир");
      }
      ...
      const errorMsg = err instanceof Error ? err.message : "Не удалось сохранить мир";
      setError(errorMsg);
      onError?.(errorMsg);
    } finally {
      setSaving(false);
    ...
    <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
      ...
      {saveSuccess && (
        <Alert severity="success">
          Мир успешно сохранён! Перенаправление...
        </Alert>
      )}
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
              ...
              <Box sx={{ display: "flex", gap: 2 }}>
                <TextField
                  ...
                  value={world.genre}
                  fullWidth
                  onChange={(e) => setWorld({ ...world, genre: e.target.value })}
                />
                <TextField
                  label="Атмосфера"
                  value={world.tone}
              ...
              <TextField
                ...
                rows={8}
                fullWidth
                onChange={(e) => setWorld({ ...world, world_primer: e.target.value })}
              />
            </Box>
          </TabPanel>
                ...
                <Accordion key={idx} defaultExpanded={idx === 0}>
                  <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                      ...
                      {faction.type && (
                        <Chip label={faction.type} size="small" variant="outlined" />
                      )}
                    </Box>
                  </AccordionSummary>
                ...
                <Accordion key={idx} defaultExpanded={idx === 0}>
                  <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                      ...
                      {event.timeframe && (
                        <Chip label={event.timeframe} size="small" variant="outlined" />
                      )}
                    </Box>
                  </AccordionSummary>
                  <AccordionDetails sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "center", mt: 2 }}>
        ...
        >
          {saving ? "Сохранение..." : saveSuccess ? "Сохранено!" : "Сохранить мир"}
        </Button>
      </Box>
    </Box>
  );
