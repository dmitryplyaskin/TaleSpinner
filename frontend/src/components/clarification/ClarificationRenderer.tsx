...
import type { ClarificationRequest, ClarificationResponse, ClarificationField } from '@shared/types/human-in-the-loop';
import {
	TextInput,
	TextareaInput,
	SliderInput,
...
export function ClarificationRenderer({ request, onSubmit, onSkip }: ClarificationRendererProps) {
	...
	const renderField = (field: ClarificationField) => {
		switch (field.type) {
			case 'text':
				return (
					<TextInput field={field} value={values[field.id] as string} onChange={(v) => handleChange(field.id, v)} />
				);
			case 'textarea':
				return (
					<TextareaInput field={field} value={values[field.id] as string} onChange={(v) => handleChange(field.id, v)} />
				);
			case 'slider':
				return (
					<SliderInput field={field} value={values[field.id] as number} onChange={(v) => handleChange(field.id, v)} />
				);
			case 'radio':
				return (
					<RadioGroup field={field} value={values[field.id] as string} onChange={(v) => handleChange(field.id, v)} />
				);
			case 'confirm':
				return (
					<ConfirmToggle
			...
			case 'select':
				return (
					<SelectInput field={field} value={values[field.id] as string} onChange={(v) => handleChange(field.id, v)} />
				);
			case 'multiselect':
				return (
					<MultiSelectInput
		...
		<div className="bg-[var(--color-background)] border border-[var(--color-border)] rounded-xl shadow-2xl overflow-hidden max-w-2xl w-full mx-auto">
			...
			<div className="bg-gradient-to-r from-[var(--color-accent)]/20 to-[var(--color-accent)]/5 px-6 py-5 border-b border-[var(--color-border)]">
				<div className="flex items-start justify-between gap-4">
					<div>
						<h2 className="text-xl font-bold text-[var(--color-text-primary)]">{request.context.title}</h2>
						<p className="text-[var(--color-text-secondary)] mt-1">{request.context.description}</p>
					</div>
					<div className="flex flex-col items-end gap-2">
						...
						<span
							className={`px-3 py-1 rounded-full text-xs font-medium border ${
								impactColors[request.meta.estimatedImpact]
							}`}
						>
			...
			<div className="px-6 py-6 space-y-6">
				{visibleFields.map((field) => (
					<div key={field.id} className="space-y-2">
						<label htmlFor={field.id} className="block text-[var(--color-text-primary)] font-medium">
							{field.label}
							{field.required && <span className="text-red-400 ml-1">*</span>}
						</label>
						{field.description && <p className="text-sm text-[var(--color-text-secondary)]">{field.description}</p>}
						{renderField(field)}
					</div>
				))}
			</div>
